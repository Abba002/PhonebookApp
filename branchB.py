...
import sqlite3
import queries2
from add_BranchB import AddEmployee
from update_branchB import Update
from display_branchB import Display
from tkinter import messagebox
from searchB import Search
...
# to create table
cur.execute(queries2.CREATE_TABLE)
conn.commit()


class BranchB(Toplevel):
    def __init__(self):
        ...
        self.heading = Label(self.top, text='Branch B Employees',
        ...
        employees = cur.execute(queries2.FETCH_ALL).fetchall()
        count = 0
    ...
    def delete_employee(self):
        ...
        if answer == 'yes':
            try:
                cur.execute(queries2.DELETE, employee_ID)
                conn.commit()
